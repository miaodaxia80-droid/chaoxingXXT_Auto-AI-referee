...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="遇到关闭任务点时的行为: retry-重试(默认), continue-跳过"
    )
...
def load_config_from_file(config_path):
    ...
    if config.has_section("common"):
        ...
        if "user_agent" in common_config:
            common_config["user_agent"] = common_config["user_agent"].strip()
...
def init_chaoxing(common_config, tiku_config):
    ...
    # 设置题库
    multi_model = str(tiku_config.get('multi_model', 'false')).lower() in ('true', '1', 'yes')
    if multi_model:
    ...
    def worker_thread(self):
        ...
        while True:
            ...
            match task.result:
                ...
                case ChapterResult.STOPPED:
                    logger.info("Task stopped: {}", task.point["title"])
                    self.task_queue.task_done()
...
def filter_courses(all_course, course_list):
    """过滤要学习的课程"""
    originally_specified = bool(course_list)
    ...
    if not course_task:
        if originally_specified:
            # 用户明确指定了课程ID但全都没匹配上 — 报错，不静默回退
            logger.error(f"指定的课程ID未匹配到任何课程: {course_list}")
            print(f"错误：指定的课程ID未找到匹配课程: {course_list}")
            print("可用课程列表：")
            for course in all_course:
                print(f"  ID: {course['courseId']} 课程名: {course['title']}")
            raise InputFormatError("指定的课程ID未匹配到任何课程")
        # 用户未指定课程列表，学习所有课程
