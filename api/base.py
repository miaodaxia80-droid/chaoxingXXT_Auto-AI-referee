...
from api.cookies import save_cookies, use_cookies, parse_cookie_string, dump_cookie_string
...
class SessionManager:
    @staticmethod
    def create_session(cookies: Optional[dict] = None) -> requests.Session:
    ...
    @staticmethod
    def resolve_cookies(account: Optional["Account"] = None) -> dict:
        if account:
            if getattr(account, "cookies_data", None):
                return parse_cookie_string(account.cookies_data)
            if not getattr(account, "use_cookie_file", True):
                return {}
        return use_cookies()
...
class Account:
    ...
    def __init__(
        ...
        self.use_cookie_file = use_cookie_file
...
class StudyResult(Enum):
    ...
    def is_failure(self):
        return self not in {StudyResult.SUCCESS, StudyResult.STOPPED}
...
class Chaoxing:
    def __init__(self, account: Account = None, tiku: Tiku = None, **kwargs):
        ...
        self.tiku = tiku
        self.kwargs = kwargs
        self.session = SessionManager.create_session(SessionManager.resolve_cookies(account))
        self.rollback_times = 0
        self.rate_limiter = RateLimiter(0.5) # 其他接口速率限制比较松
        self.video_log_limiter = RateLimiter(2) # 上报进度极其容易卡验证码，限制2s一次
        self._last_quiz_low_coverage = False  # 标记最近一次答题是否因覆盖率不足未提交
    ...
    def login(self, login_with_cookies=False):
        if login_with_cookies:
            ...
            SessionManager.refresh_session_cookies(self.session, self.account)
        ...
        if self.account and getattr(self.account, "user_agent", None):
            _session.headers["User-Agent"] = self.account.user_agent
        ...
        if resp_json.get("status") == True:
            should_save_cookie_file = bool(
                self.account is None or getattr(self.account, "use_cookie_file", True)
            ) and not bool(self.account and self.account.cookies_data)
            self.session.cookies.clear()
            self.session.cookies.update(_session.cookies)
            if self.account is not None:
                self.account.cookies_data = dump_cookie_string(_session)
            if should_save_cookie_file:
    ...
    def get_course_list(self):
        ...
        if 'passport2.chaoxing.com' in _resp.text or '用户登录' in _resp.text:
            logger.warning('课程列表接口返回登录页，session 可能已过期')
            return []
    ...
    def study_video(self, _course, _job, _job_info, _speed: float = 1.0, _type: Literal["Video", "Audio"] = "Video") -> StudyResult:
        ...
        while not passed:
            # 响应 Web 面板的停止信号
            if getattr(self, '_task_stop_event', None) and self._task_stop_event.is_set():
                logger.info("收到停止信号，中断视频任务: {}", _job['name'])
                return StudyResult.STOPPED
    ...
    def study_document(self, _course, _job) -> StudyResult:
        _session = self.session
        try:
            _node_match = re.findall(r'nodeId_(.*?)-', _job['otherinfo'])
            if not _node_match:
                logger.warning("文档任务 otherinfo 格式异常: {}", _job.get('otherinfo',''))
                _node_match = re.findall(r'nodeId_(.*?)$', _job.get('otherinfo',''))
            _knowledge_id = _node_match[0] if _node_match else ''
            _url = f"https://mooc1.chaoxing.com/ananas/job/document?jobid={_job['jobid']}&knowledgeid={_knowledge_id}&courseid={_course['courseId']}&clazzid={_course['clazzId']}&jtoken={_job['jtoken']}&_dc={get_timestamp()}"
        except Exception as e:
            logger.error("文档任务 URL 构建失败: {}", e)
            return StudyResult.ERROR
        _resp = _session.get(_url)
        if _resp.status_code != 200:
            logger.error("文档任务请求失败 [{}]: {}", _resp.status_code, _resp.text[:200])
            return StudyResult.ERROR
        else:
            logger.info("文档任务完成: {}", _job.get('name', _job.get('jobid','')))
            return StudyResult.SUCCESS
    ...
    def study_work(self, _course, _job, _job_info) -> StudyResult:
        ...
        if self.tiku.get_submit_params() == "1":
            ...
            questions["pyFlag"] = "1"
            self._last_quiz_low_coverage = True  # 标记未提交，供 web 端人工接管检测
    ...
    def study_read(self, _course, _job, _job_info) -> StudyResult:
        ...
        try:
            _resp = _session.get(
                ...
                params={
                    "jobid": _job["jobid"],
                    "knowledgeid": _job_info.get("knowledgeid", ""),
                    "jtoken": _job["jtoken"],
                    "courseid": _course["courseId"],
                    "clazzid": _course["clazzId"],
                },
            ...
            if _resp.status_code != 200:
                logger.error(f"阅读任务失败 [{_resp.status_code}]: {_resp.text[:200]}")
                return StudyResult.ERROR
            _resp_json = _resp.json()
            logger.info(f"阅读任务完成: {_resp_json.get('msg','')}")
            return StudyResult.SUCCESS
        except Exception as e:
